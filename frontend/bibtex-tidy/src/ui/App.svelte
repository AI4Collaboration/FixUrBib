...
// LLM settings
let apiKey: string = "";
let apiEndpoint: string = "http://localhost:8000";
...
function splitBibtexEntries(bibtexStr: string): string[] {
...
async function handleLLMFix() {
	...
	try {
		...
		if (entries.length === 0) {
			status = { status: "llm-error", error: "No BibTeX entries found" };
			return;
		}

		const response = await fetch(`${apiEndpoint}/api/fix-batch`, {
			method: "POST",
			headers: {
		...
		if (!data.success) {
			status = { status: "llm-error", error: data.error || "Unknown error" };
			return;
		}

		// Combine fixed entries
		const fixedEntries = data.results
			.map((r: { success: boolean; result?: string; error?: string }) =>
				r.success ? r.result : null
			)
			.filter((r: string | null): r is string => r !== null);

		bibtex = fixedEntries.join("\n\n");
		status = { status: "llm-success", count: fixedEntries.length };
	} catch (e: unknown) {
		console.error("LLM fix error:", e);
		status = {
			status: "llm-error",
