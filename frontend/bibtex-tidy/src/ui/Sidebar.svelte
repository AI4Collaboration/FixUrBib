...
export let llmRunning: boolean = false;
...
let dispatch = createEventDispatcher<{ tidy: undefined; llmfix: undefined }>();
</script>
...
<aside id="sidebar">
	<form on:submit={() => false}>
		...
		<LLMOptions bind:apiKey bind:apiEndpoint bind:model />
	...
	<div class="run">
		...
		<div class="button-group">
			<button id="llmfix" class="llm-btn" disabled={llmRunning || !apiKey} on:click={() => dispatch('llmfix')}>
				{llmRunning ? 'Fixing...' : 'LLM Fix'}
			</button>
...
<style>
	...
	#sidebar #llmfix[disabled] {
		background: var(--dark-gray);
		color: transparent;
		position: relative;
	}
	#sidebar #llmfix:not([disabled]) {
		color: var(--main-bg);
	}
	#sidebar #tidy[disabled]:after,
	#sidebar #llmfix[disabled]:after {
